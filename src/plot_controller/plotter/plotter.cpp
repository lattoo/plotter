...
void Plotter::replot(QPainter* painter)
{
    ...
    _grid.x().replot(painter, _x_ticks_main_grid);
    _grid.y().replot(painter, _y_ticks_main_grid);

    _grid.sub_x().replot(painter, _x_ticks_sub_grid);
    _grid.sub_y().replot(painter, _y_ticks_sub_grid);
...
//    _grid.sub_grid().grid_y().replot(painter, _y_ticks_sub_grid);
    ...
    _axes.terminators().x().replot(painter, QPointF(bottom_right.x(), rescale_to_y_pixels(0)));
    _axes.terminators().y().replot(painter, top_left);

    _axes.ticks().x().replot(painter, _x_ticks_pf);
    _axes.ticks().y().replot(painter, _y_ticks_pf);
    _axes.ticks().sub_x().replot(painter, _x_subticks_pf);
    _axes.ticks().sub_y().replot(painter, _y_subticks_pf);

    _axes.zero_lines().x().replot(painter, _x_zero_line);
    _axes.zero_lines().y().replot(painter, _y_zero_line);
...
{
    const float x_line_y = rescale_to_y_pixels(0);

    _x_zero_line = QLineF(_plot_area_borders.left(), x_line_y,
                          _plot_area_borders.right() + AXES_EXTENSION, x_line_y);
